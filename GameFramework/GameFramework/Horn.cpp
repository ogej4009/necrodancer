...
int Horn::NumberCount = 0;

Horn::Horn(CVector _TargetPos) : m_Direction(true), m_MyCount(0), m_Changed(false)
, m_DelayTime(0)
{
	m_ActorTypeIndex = EActorTypeIndex::M_CoralHorn;


	Numbering[0] = L"coralriff_horns.bmp";
	Numbering[1] = L"coralriff_horns2.bmp";
	...
	m_MonsterPosIndex = m_CoralHorn->SubPosIndeX();
...
void Horn::Update()
{
	if (15 < CoralHead::GlobalTime && CoralHead::GlobalTime < 18)
	{
		if (!m_Changed)
		{
			m_Changed = true;
...
void Horn::ChangePos(CVector _Pos)
{
	m_CoralHorn->SubPos(_Pos);
	CVector subPosPure = _Pos;
	subPosPure.Y -= m_TileSizeHalf;
	m_CoralHorn->SubPosPure(subPosPure);
...
void Horn::InitAni(CVector _TargetPos)
{
	if (NumberCount > 1)
	{
		NumberCount = 0;
	...
	m_CoralHorn = CreateRender<AniRender>(static_cast<int>(m_ActorTypeIndex));
	m_CoralHorn->CreateAni(IDLE, Numbering[NumberCount].c_str(), 0, 3, true, 0.1f);
	m_CoralHorn->CreateAni(ShadowIDLE, Numbering[NumberCount].c_str(), 4, 7, true, 0.1f);
	m_CoralHorn->SubPos(_TargetPos);
	CVector subPosPure = _TargetPos;
	subPosPure.Y -= m_TileSizeHalf;
	m_CoralHorn->SubPosPure(subPosPure);
	m_CoralHorn->SubSize({ m_TileSize *1.5f , m_TileSize * 2 });
...
void Horn::ChangeState(MonsterState _State)
{
	m_EState = _State;
	m_CoralHorn->ChangeAni(_State);
}