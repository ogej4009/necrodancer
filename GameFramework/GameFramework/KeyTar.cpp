...
int Keytar::NumberCount = 0;

Keytar::Keytar(CVector _TargetPos) : m_Direction(true), m_MyCount(0), m_Changed(false)
, m_DelayTime(0)
{
	m_ActorTypeIndex = EActorTypeIndex::M_CoralKeytar;


	Numbering[0] = L"coralriff_keytar.bmp";
	Numbering[1] = L"coralriff_keytar2.bmp";
	...
	m_MonsterPosIndex = m_CoralKeytar->SubPosIndeX();
...
void Keytar::Update()
{
	if (5 < CoralHead::GlobalTime && CoralHead::GlobalTime < 8)
	{
		if (!m_Changed)
		{
			m_Changed = true;
			...
			{
			case 0:
				firstIndex = { State()->GetPlayer()->PlayerIndex.X - 1 ,  State()->GetPlayer()->PlayerIndex.Y };
				break;
			case 1:
				firstIndex = { State()->GetPlayer()->PlayerIndex.X + 1,  State()->GetPlayer()->PlayerIndex.Y  };
			default:
...
void Keytar::ChangePos(CVector _Pos)
{
	m_CoralKeytar->SubPos(_Pos);
	CVector subPosPure = _Pos;
	subPosPure.Y -= m_TileSizeHalf;
	m_CoralKeytar->SubPosPure(subPosPure);
...
void Keytar::InitAni(CVector _TargetPos)
{
	if (NumberCount > 1)
	{
		NumberCount = 0;
	...
	m_CoralKeytar = CreateRender<AniRender>(static_cast<int>(m_ActorTypeIndex));
	m_CoralKeytar->CreateAni(IDLE, Numbering[NumberCount].c_str(), 0, 3, true, 0.1f);
	m_CoralKeytar->CreateAni(ShadowIDLE, Numbering[NumberCount].c_str(), 4, 7, true, 0.1f);
	m_CoralKeytar->SubPos(_TargetPos);
	CVector subPosPure = _TargetPos;
	subPosPure.Y -= m_TileSizeHalf;
	m_CoralKeytar->SubPosPure(subPosPure);
	m_CoralKeytar->SubSize({ m_TileSize *1.5f , m_TileSize * 2 });
...
void Keytar::ChangeState(MonsterState _State)
{
	m_EState = _State;
	m_CoralKeytar->ChangeAni(_State);
}