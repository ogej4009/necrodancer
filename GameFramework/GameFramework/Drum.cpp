...
int Drum::NumberCount = 0;


Drum::Drum(CVector _TargetPos)
	: m_Direction(true)
	, m_DelayTime(0)
	, m_MyCount(0)
	, m_Changed(false)
{
	m_ActorTypeIndex = EActorTypeIndex::M_CoralDrum;


	Numbering[0] = L"coralriff_drums.bmp";
	Numbering[1] = L"coralriff_drums2.bmp";
	...
	m_MonsterPosIndex = m_CoralDrum->SubPosIndeX();
...
void Drum::Update()
{
	if (20 < CoralHead::GlobalTime && CoralHead::GlobalTime < 23)
	{
		if (!m_Changed)
		{
			m_Changed = true;
...
void Drum::ChangePos(CVector _Pos)
{
	m_CoralDrum->SubPos(_Pos);
	CVector subPosPure = _Pos;
	subPosPure.Y -= m_TileSizeHalf;
	m_CoralDrum->SubPosPure(subPosPure);
...
void Drum::InitAni(CVector _TargetPos)
{
	if (NumberCount > 1)
	{
		NumberCount = 0;
	...
	m_CoralDrum = CreateRender<AniRender>(static_cast<int>(m_ActorTypeIndex));
	m_CoralDrum->CreateAni(IDLE, Numbering[NumberCount].c_str(), 0, 3, true, 0.1f);
	m_CoralDrum->CreateAni(ShadowIDLE, Numbering[NumberCount].c_str(), 4, 7, true, 0.1f);
	m_CoralDrum->SubPos(_TargetPos);
	CVector subPosPure = _TargetPos;
	subPosPure.Y -= m_TileSizeHalf;
	m_CoralDrum->SubPosPure(subPosPure);
	m_CoralDrum->SubSize({ m_TileSize *1.5f , m_TileSize * 2 });
...
void Drum::ChangeState(MonsterState _State)
{
	m_EState = _State;
	m_CoralDrum->ChangeAni(_State);
}